  ...
  return (
    <div className="bg-[#16161C] border border-[#D4AF37]/30 rounded-[2.5rem] p-10 shadow-[0_25px_60px_rgba(0,0,0,0.8)] backdrop-blur-2xl w-full max-w-md mx-auto relative overflow-hidden text-white">
      ...
      <div className="flex justify-between items-center mb-8 relative z-10">
        <h2 className="text-[22px] font-black tracking-widest text-white uppercase">GİRİŞ YAP</h2>
        <button type="button" onClick={() => setView('set-role')} className="text-white/50 hover:text-[#D4AF37] transition-colors p-2 rounded-xl bg-white/5 border border-white/5 hover:border-[#D4AF37]/30">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
        ...
        <div className="relative">
          ...
          <button 
            ...
            className="absolute right-5 inset-y-0 flex items-center text-white/40 hover:text-[#D4AF37] transition-colors"
          >
            {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
          </button>
        </div>

        <div className="flex justify-between items-center px-2 text-[10px] font-bold text-white/50 tracking-wide pt-1">
          <label className="flex items-center gap-2 cursor-pointer hover:text-white transition-colors">
            <input 
              type="checkbox" 
              checked={rememberMe}
              onChange={(e) => setRememberMe(e.target.checked)}
              className="w-3.5 h-3.5 rounded bg-[#221F1C] text-[#D4AF37] focus:ring-0 border-[#D4AF37]/30 cursor-pointer accent-[#D4AF37]"
            />
            <span>Beni Hatırla</span>
          </label>
          <a href="#" className="hover:text-[#D4AF37] underline transition-colors">Şifremi Unuttum?</a>
        </div>
      ...
      <div className="text-center mt-8 relative z-10">
        <p className="text-[10px] text-white/50 font-bold tracking-wide">
          Hesabınız yok mu?{' '}
          <button type="button" onClick={() => setView('client-register')} className="text-[#D4AF37] hover:text-amber-300 hover:underline font-black uppercase ml-1 transition-colors">
            Kayıt Ol
          </button>
        </p>
      </div>
