...
function ContactContent() {
  ...
  // 1. Kullanıcı Oturum Bilgisini Çekme (/api/auth/me)
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.warn("Kullanıcı oturumu doğrulanamadı:", res.status);
        }
      } catch (err) {
        console.error("Auth me isteği başarısız:", err);
      } finally {
  ...
  // 2. MESAJ SAYISINI GERÇEK BACKEND'DEN HESAPLA
  ...
  // 3. URL ?tab= parametresini yakalama & Senkronizasyon
  useEffect(() => {
  ...
  // 4. CHILD COMPONENT'LERDEN SAYILARI YAKALAMA
  const handleAppointmentsCount = useCallback((count) => {
    setUnreadCounts((prev) => ({
      ...prev,
      appointments: Number.isFinite(Number(count)) ? Number(count) : 0
    }));
  }, []);

  const handleSupportCount = useCallback((count) => {
    setUnreadCounts((prev) => ({
      ...prev,
      support: Number.isFinite(Number(count)) ? Number(count) : 0
    }));
  }, []);
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 bg-[#0B1310]/95 border-2 border-emerald-500/40 hover:border-emerald-400/70 p-5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.8),0_0_30px_rgba(16,185,129,0.15)] backdrop-blur-2xl relative overflow-hidden transition-all duration-300">
          ...
          <div className="relative flex bg-[#060B09] p-1.5 sm:p-2 rounded-2xl w-full md:w-auto shadow-inner border-2 border-emerald-950 z-10 gap-1.5 overflow-x-auto no-scrollbar">
            ...
            >
              ...
              {unreadCounts.appointments !== null && unreadCounts.appointments > 0 && (
                <span
            ...
            >
              ...
              {unreadCounts.support !== null && unreadCounts.support > 0 && (
                <span
                  className={`px-1.5 py-0.5 text-[9px] font-mono font-extrabold rounded-full ${
                    activeTab === "support"
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6 w-full flex-grow mt-2">
        {loadingUser ? (
          <div className="flex items-center justify-center py-20 text-emerald-400 gap-3">
            ...
            <span className="text-xs font-black tracking-widest uppercase">Kullanıcı Oturumu Doğrulanıyor...</span>
          </div>
        ) : (
          <>
            {activeTab === "appointments" && (
              <AppointmentsTab 
                ...
                onCountChange={handleAppointmentsCount}
              />
            )}
            {activeTab === "messages" && (
              <MessagesTab 
            ...
            {activeTab === "support" && (
              <SupportTab 
                currentUser={currentUser} 
                onCountChange={handleSupportCount}
              />
            )}
          </>
        )}
