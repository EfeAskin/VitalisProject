...
import { Plus, Dumbbell, Clock, Edit3, Trash2, Send, Zap, Utensils, Apple, Loader2 } from 'lucide-react';
...
function ProgramsContent() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const formattedWorkouts = rawList.map(t => ({
          ...
          level: t.difficulty_level || t.level || 'Başlangıç',
          duration: t.duration_minutes || t.duration || 45,
          targetMuscles: t.target_muscles || t.targetMuscles || [],
          exercises: (t.exercises || []).map(e => {
            ...
            if (videoUrl) {
              if (videoUrl.includes("/static/uploads/")) {
                mediaType = "video";
                mediaLink = videoUrl;
              } else if (videoUrl.startsWith("http://") || videoUrl.startsWith("https://") || videoUrl.includes("youtube") || videoUrl.includes("youtu.be")) {
                mediaType = "youtube";
                mediaLink = videoUrl;
              } else {
                mediaType = "youtube";
      ...
      console.error("Şablonlar veritabanından çekilirken hata oluştu:", err);
    } finally {
  ...
  useEffect(() => {
    if (userRole === 'trainer' && activeTab === 'workout-templates') {
      fetchWorkouts();
    }
  }, [userRole, activeTab]);
  ...
  const handleEditWorkout = async (workout) => {
    try {
      const token = localStorage.getItem("token") || localStorage.getItem("access_token");
      const res = await fetch(`/api/expert/workout-templates/${workout.id}`, {
        headers: {
          'Authorization': token ? `Bearer ${token}` : '',
          'Content-Type': 'application/json'
        }
      ...
      if (res.ok) {
        ...
        const t = detailData.template || detailData.data || detailData;

        if (t) {
          const formattedEditWorkout = {
            id: t.id,
            title: t.name || t.title || workout.title,
            level: t.difficulty_level || t.level || workout.level,
            duration: t.duration_minutes || t.duration || workout.duration,
            targetMuscles: t.target_muscles || t.targetMuscles || workout.targetMuscles || [],
            exercises: (t.exercises || []).map(e => {
              const videoUrl = e.video_url || "";
              let mediaType = "none";
              let mediaLink = "";
      ...
      console.error("Şablon detayı alınırken hata oluştu:", err);
      setEditingWorkout(workout);
      setIsWorkoutBuilderOpen(true);
    }
  };
  ...
  const handleDeleteWorkout = async (templateId) => {
    if (!confirm("Bu antrenman şablonunu silmek istediğinizden emin misiniz?")) return;
    try {
      const token = localStorage.getItem("token") || localStorage.getItem("access_token");
      const res = await fetch(`/api/expert/workout-templates/${templateId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': token ? `Bearer ${token}` : ''
        }
          ...
          <>
            {isLoadingWorkouts ? (
              <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                <Loader2 size={36} className="animate-spin text-[#EA580C] mb-3" />
                <p>Veritabanından antrenman şablonları yükleniyor...</p>
              </div>
            ) : workouts.length === 0 ? (
                  ...
                  <div key={workout.id} className="bg-[#1A1F37]/60 backdrop-blur-md border border-white/10 hover:border-orange-500/50 rounded-3xl p-6 shadow-[0_10px_30px_rgba(0,0,0,0.3)] transition-all flex flex-col justify-between group hover:shadow-[0_0_20px_rgba(234,88,12,0.1)]">
                    <div>
                      ...
                      <div className="flex items-center gap-6 text-xs font-medium text-slate-400 mb-6">
                        ...
                        <span className="flex items-center gap-2"><Dumbbell size={16} className="text-orange-500" /> {workout.exercises?.length || 0} Egzersiz</span>
                      </div>
