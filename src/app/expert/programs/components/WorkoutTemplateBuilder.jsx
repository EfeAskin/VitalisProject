...
const isMuscleMatch = (targetKey, exerciseMuscleGroup, exerciseTargets) => {
  ...
  return mappedKeywords.some((keyword) => {
    ...
    if (
      normGroup.includes(normKeyword) ||
      normKeyword.includes(normGroup)
    ) {
      return true;
    }
    ...
    return normTargets.some(
      (t) =>
        t.includes(normKeyword) ||
        normKeyword.includes(t)
    );
  });
};
...
export default function WorkoutTemplateBuilder({
  ...
  useEffect(() => {
    ...
    const fetchAvailableExercises = async () => {
      ...
      try {
        ...
        console.error(
          "Önerilen hareketler yüklenirken hata oluştu:",
          err
        );
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      setEstimatedCalories(
        initialData.estimated_calories !== undefined &&
          initialData.estimated_calories !== null
          ? String(initialData.estimated_calories)
          : initialData.estimatedCalories !== undefined &&
            initialData.estimatedCalories !== null
          ? String(initialData.estimatedCalories)
          : ""
      );

      setTargetMuscles(
        initialData.targetMuscles ||
          initialData.target_muscles ||
          []
      );
  ...
  const handleFileSelect = (
    ...
    if (
      ...
      alert(
        "Sadece .mp4, .webm veya .gif uzantılı dosyalar yüklenebilir."
      );
      return;
    }
  ...
  const handleSelectSuggestedExercise = (
    rowId,
    suggestedEx
  ) => {
    const videoUrl =
      suggestedEx.video_url ||
      suggestedEx.videoUrl ||
      suggestedEx.mediaLink ||
      "";
  ...
  const filteredSuggestionsForBar =
  ...
  const handleSave = async () => {
    try {
      ...
      // Zorluk seviyesini seçildiği haliyle gönder
      formData.append(
        "level",
        level
      );
      ...
      // Kaloriyi tek bir güvenilir numeric değer olarak gönder
      const caloriesValue =
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.detail ||
            errorData.message ||
            "Şablon kaydedilirken bir sunucu hatası oluştu."
        );
      }
      ...
      <div className="bg-[#11142D] border border-slate-700/80 w-full max-w-4xl max-h-[85vh] overflow-y-auto rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col text-sm text-slate-100 selection:bg-orange-500 selection:text-white -translate-y-11">
        ...
        <div className="sticky top-0 bg-[#11142D]/95 backdrop-blur-xl z-20 flex justify-between items-center px-6 py-5 border-b border-slate-700/50">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Elit danışanlarınız için kusursuz bir program tasarlayın.
            </p>
          </div>
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="bg-[#181C3B]/80 border border-slate-700/60 rounded-2xl p-4 space-y-2.5 shadow-inner">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-amber-300 tracking-wide uppercase">
                  ...
                  0
                    ? "(Seçili Kas Grubu İçin)"
                    : ""}
                </span>
              </div>

              <span className="text-[11px] text-slate-400">
                Tıklayarak programa ekleyebilirsiniz
              </span>
            </div>
            ...
            ) : filteredSuggestionsForBar.length >
              0 ? (
              <div className="flex flex-wrap gap-2 pt-1">
                {filteredSuggestionsForBar
                  .slice(0, 8)
                  .map(
                    (sugEx) => {
              ...
              {exercises.map(
                (ex, index) => {
                  ...
                  return (
                    <div
                      key={ex.id}
                      className="bg-[#11142D]/40 border border-slate-700/50 rounded-2xl p-5 relative group backdrop-blur-sm shadow-md"
                    >
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 mt-1.5">
                        ...
                        {/* Hareket Adı */}
                        <div
                          className="lg:col-span-4 space-y-1 relative"
                          ref={(el) =>
