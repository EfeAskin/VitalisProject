...
export default function WorkoutCard({ workout, onEdit, onDelete, onAssign }) {
  ...
  return (
    <div className="bg-[#111827] border border-slate-800 rounded-3xl p-6 shadow-xl hover:shadow-2xl hover:border-orange-500/40 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between group relative overflow-hidden">
      ...
      <div>
        ...
        <div className="flex justify-between items-start mb-4">
          <span className={`text-[10px] font-black px-3 py-1.5 rounded-lg border uppercase tracking-wider ${getLevelStyle(level)}`}>
            {level}
          </span>
          ...
          <div className="flex items-center gap-2 opacity-40 group-hover:opacity-100 transition-opacity duration-300">
            ...
            <button 
              ...
              className="p-2 bg-slate-800 hover:bg-red-500/20 text-slate-300 hover:text-red-400 rounded-lg transition-colors"
              title="Şablonu Sil"
            >
              <Trash2 size={15} />
            </button>
        ...
        {/* Metrikler */}
        <div className="flex flex-wrap items-center gap-4 text-xs font-semibold text-slate-400 mb-5">
        ...
        <div className="bg-[#182134] p-4 rounded-2xl border border-slate-700/50 mb-5 space-y-2.5">
          {exercises.slice(0, 3).map((ex, i) => (
            <div key={ex.id || i} className="flex justify-between items-center text-xs">
              <span className="font-medium text-slate-300 truncate max-w-[170px] flex items-center gap-2">
                <span className="w-1 h-1 rounded-full bg-[#EA580C]"></span> {ex.name || "İsimsiz Egzersiz"}
              </span>
              <span className="font-bold text-slate-500 text-[11px] bg-[#0B1120] px-2 py-1 rounded-md border border-slate-700/50">
                {ex.sets} x {ex.reps}
              </span>
            </div>
          ))}
          ...
          {exercises.length === 0 && (
            <p className="text-xs text-slate-500 italic text-center py-2">Egzersiz eklenmedi.</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-auto">
        ...
        <div className="flex items-center justify-between mb-4 px-1">
          ...
          <div className="flex -space-x-2">
            {assignedUsers.slice(0, 3).map((u, idx) => (
              <div key={idx} className="w-6 h-6 rounded-full border-2 border-[#111827] bg-slate-700 flex items-center justify-center text-[8px] font-bold text-white z-10">
                PT
              </div>
            ))}
            {assignedUsers.length > 3 && (
              <div className="w-6 h-6 rounded-full border-2 border-[#111827] bg-[#182134] flex items-center justify-center text-[8px] font-bold text-slate-400 z-0">
                +{assignedUsers.length - 3}
              </div>
            )}
            {assignedUsers.length === 0 && (
              <span className="text-[10px] text-slate-500">Henüz kimseye atanmadı</span>
            )}
          </div>
        </div>
        ...
        <button 
          ...
          className="w-full flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-slate-800 to-slate-900 hover:from-[#EA580C] hover:to-orange-500 text-white text-xs font-bold rounded-xl shadow-lg transition-all duration-300"
        >
