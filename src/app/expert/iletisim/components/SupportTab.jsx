...
export default function SupportTab() {
  ...
  const handleCreateTicket = (e) => {
    ...
    setNewTicket({ subject: "", category: "Teknik & Platform Hatası", priority: "Normal", message: "" });
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/60 border border-slate-800 p-6 rounded-3xl shadow-xl">
        <div>
          <h3 className="text-lg font-black text-white flex items-center gap-2">
            <Headphones className="w-5 h-5 text-purple-500" /> Platform & Uzmanlık Desteği (Ticket)
          </h3>
          <p className="text-xs text-slate-400 mt-0.5">
            Ödemeler, danışan yönetimi veya sistem hatalarıyla ilgili platform yönetiminden 7/24 admin desteği alın.
          </p>
        </div>
        <button
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 md:p-8 shadow-2xl relative space-y-6">
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-black text-white flex items-center gap-2">
                <ShieldAlert className="w-5 h-5 text-purple-500" /> Admin Destek Talebi
              </h3>
              <button 
