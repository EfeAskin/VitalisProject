...
router = APIRouter(
    prefix="/api/expert/profile",
    tags=["Expert Profile Management"]
)
...
def get_expert_profile(
    ...
    """
    Giriş yapmış uzman kullanıcısının (Trainer/Dietitian) Neon DB üzerindeki profil verilerini getirir.
    """
    ...
    try:
        ...
        if not user_data:
            raise HTTPException(
                ...
                detail="Uzman veritabanında bulunamadı."
            )
        ...
        raise HTTPException(
            ...
            detail=f"Veritabanından uzman profili çekilirken hata oluştu: {str(e)}"
        )
...
def update_expert_profile(
    ...
    """
    Uzman kullanıcının profil ve kişisel verilerini Neon DB'de dinamik olarak günceller.
    """
    user_id = current_user.get("id")
    if not user_id:
        raise HTTPException(
    ...
    try:
        ...
        if not updated_user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Güncellenecek uzman kaydı bulunamadı."
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Uzman profili güncellenirken veritabanı hatası: {str(e)}"
        )
...
async def upload_expert_profile_photo(
    ...
    """
    Uzman profil fotoğrafını sunucu diski/public dizinine kaydeder ve Neon DB'deki profile_photo alanını günceller.
    """
    user_id = current_user.get("id")
    if not user_id:
        raise HTTPException(
    ...
    try:
        ...
        return {
            "message": "Uzman profil fotoğrafı başarıyla yüklendi ve veritabanı güncellendi.",
            "photo_url": photo_url
        }
