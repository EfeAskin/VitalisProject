...
#
# ÖNEMLİ:
# Veritabanı seçimi database.py tarafından yapılmaktadır.
#
# database.py:
#   - Neon erişilebiliyorsa Neon
#   - Neon erişilemiyorsa Local
#   - DATABASE_MODE local ise Local
#   - DATABASE_MODE neon ise Neon
#
# Bu router kendi DB seçimini YAPMAZ.
# Merkezi database.py içindeki get_db_connection kullanılır.
# ============================================================
    ...
    try:
        ...
        for u in users_raw:
            ...
            full_name = (
                ...
                f"{u.get('last_name') or ''}"
            ).strip()

            if not full_name:
                full_name = (
                    u.get("email") or "Uzman"
                ).split("@")[0].capitalize()
            ...
            expert_obj = {
                ...
                "rating": float(
                    prof.get("rating") or 5.0
                ),
                "reviewCount": int(
                    prof.get("review_count") or 12
                ),
                "experienceYears": 5,
                "minPrice": min_price,
                "specialties": specs,
            ...
            matches_search = (
                not search_term
                or search_term
                in expert_obj["name"].lower()
                or search_term
                in expert_obj["title"].lower()
                or any(
